...
df = pd.read_csv('blinded_soms_outlier_removed_parameters.csv', header=0)
df['total_err'] = df['q_err'] + df['t_err']
df['err_diff'] = abs(df['q_err'] - df['t_err'])

# median quantisation errors
q_err_split_array = np.array(np.split(df["q_err"], 125))
...
q_err_med = quant_errors.nsmallest(10, 'med_quant_err', keep='all')
q_err_med_list = sorted(q_err_med['family'].values.tolist())

# median topographic errors
t_err_split_array = np.array(np.split(df["t_err"], 125))
...
t_err_med = top_errors.nsmallest(10, 'med_top_err', keep='all')
t_err_med_list = sorted(t_err_med['family'].values.tolist())

# median total errors
tot_err_split_array = np.array(np.split(df["total_err"], 125))
...
tot_err_med = total_errors.nsmallest(125, 'med_total_err', keep='all')
tot_err_med_list = sorted(tot_err_med['family'].values.tolist())

# median error difference
err_diff_split_array = np.array(np.split(df["err_diff"], 125))
...
err_diff_med = error_differences.nsmallest(10, 'med_err_diff', keep='all')
...
df_sort = selected_data.sort_values(by=['total_err'])
df_lol = df_sort.values.tolist()
for i in range(0, 20):
    print(f"{df_lol[i][0]} & {df_lol[i][1]} & {df_lol[i][3]} & {df_lol[i][4]} & {df_lol[i][5]} & {df_lol[i][6]}")
